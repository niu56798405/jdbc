...
public class ListSetCodec<T extends Collection<V>, V> implements FieldCodec<T, String> {
    ...
    public static <T> FieldCodec<T, String> fetchCodec(Field field) {
        ...
        int dels = an == null ? Configuration.getDefaultDelimiters() : an.value();
        char del = (char) (CodecUtil.unpack1(dels, Configuration.getDefaultDelimiters()));
        return new ListSetCodec(CodecUtil.getCollectionFactory(factory), CodecUtil.getElementCodec(genericType), Character.toString(del));
    }
